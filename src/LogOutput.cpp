...
void CLogOutput::write_to_file(TLogData* logdata)
{
    ...
    {
         m_stream[logdata->logtype]->open(m_typename.at(logdata->logtype).c_str()
             , std::ios::out|std::ios::app);
    }
    (*m_stream[logdata->logtype])<<CPubTools::get_timestr().c_str()
          <<" "<<m_leveltext[logdata->loglevel].c_str()<<datainfo.c_str()<<std::endl;
    //输出缓存
